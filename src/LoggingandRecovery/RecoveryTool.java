...
public class RecoveryTool {
    ...
    public RecoveryTool() throws Exception {
...
//			}
        ...
        for (int i = 0; i < this.LogDatenRead.size(); i++) {
            ...
            LSN = Integer.valueOf(tmpLog.substring(0, erstLogCommaPosition).trim());
            if (ifCommited.equalsIgnoreCase("committeds")) {
                int tmpTaid = Integer.valueOf(tmpLog.substring(erstLogCommaPosition + 1, lastLogCommaPosition));
                ...
                logPageid = Integer.valueOf(tmpLog.substring(zweiteLogCommaPosition + 1, dritteLogCommaPosition));
        ...
        for (int i = 0; i < this.LogDatenRead.size(); i++) {
            ...
            if (commitlLogSN.contains(LSN)) {
                ...
                int zweiteLogCommaPosition = tmpLog.indexOf(",", erstLogCommaPosition + 1);
                int dritteLogCommaPosition = tmpLog.indexOf(",", zweiteLogCommaPosition + 1);
                taid = Integer.valueOf(tmpLog.substring(erstLogCommaPosition + 1, zweiteLogCommaPosition));
                logPageid = Integer.valueOf(tmpLog.substring(zweiteLogCommaPosition + 1, dritteLogCommaPosition));
...
//							a.write(taid, logPageid, data,true,LSN);	
    ...
    public void rcvInsert(File inFileNutz, int pageID, String nutzDatenToBeRecov) throws Exception {
        if (this._TMP_DATEN == null) {
            this._TMP_DATEN = new File("tmpdaten.txt");
        }
